/**
 * Particles — WebGL floating particle field.
 * Source: https://www.reactbits.dev/backgrounds/particles (MIT licence)
 * Adapted to TypeScript. CSS inlined — no separate stylesheet needed.
 */
...
export function Particles({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ dpr: window.devicePixelRatio || 1, depth: false, alpha: true })
    ...
    // Listen on window so the cursor is tracked even when hovering over
    // content elements (text, buttons) that sit above the canvas layer.
    ...
    const palette   = particleColors && particleColors.length > 0 ? particleColors : ['#0080FF']
